...
-- ----------------------------
BEGIN;
INSERT INTO `__PREFIX__security_data_recycle` VALUES ('1', '管理员', 'auth/Admin.php', '', '__PREFIX__admin', 'id', '1', '1648958789', '1648958789');
...
INSERT INTO `__PREFIX__security_data_recycle` VALUES ('3', '菜单规则', 'auth/Menu.php', '', '__PREFIX__menu_rule', 'id', '1', '1648959494', '1648959494');
INSERT INTO `__PREFIX__security_data_recycle` VALUES ('4', '系统配置项', 'routine/Config.php', '', '__PREFIX__config', 'id', '1', '1648959518', '1648959510');
...
CREATE TABLE `__PREFIX__security_sensitive_data_log` (
    ...
    `before` varchar(100) NOT NULL DEFAULT '' COMMENT '修改前',
    `after` varchar(100) NOT NULL DEFAULT '' COMMENT '修改后',
    `ip` varchar(50) NOT NULL DEFAULT '' COMMENT '操作者IP',
    `useragent` varchar(255) NOT NULL DEFAULT '' COMMENT 'User Agent',
...
CREATE TABLE `__PREFIX__test_build` (
    `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID',
    `admin_id` int(10) DEFAULT '0' COMMENT '管理员(远程select)',
    `user_id` int(10) DEFAULT '8' COMMENT '会员(远程select)',
    `user_ids` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '8,7' COMMENT '会员多(远程selects)',
    `title` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT '' COMMENT '标题(String)',
    `views` int(10) unsigned DEFAULT '0' COMMENT '浏览量(Number-步进1)',
    `price` decimal(10,2) unsigned DEFAULT '0.00' COMMENT '价格(Number-步进0.01)',
    `week` enum('monday','tuesday','wednesday') COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '星期(Radio-默认NULL):monday=星期一,tuesday=星期二,wednesday=星期三',
    `flag` set('hot','index','recommend') COLLATE utf8mb4_unicode_ci DEFAULT '' COMMENT '标志(Checkbox-无默认值):hot=热门,index=首页,recommend=推荐',
    `hobbydata` set('music','reading','swimming') COLLATE utf8mb4_unicode_ci DEFAULT 'reading,swimming' COMMENT '爱好(Checkbox-默认读书游泳):music=音乐,reading=读书,swimming=游泳',
    `switch` tinyint(1) DEFAULT '0' COMMENT '开关(Switch-默认关)',
    `toggle` enum('1','0') COLLATE utf8mb4_unicode_ci DEFAULT '1' COMMENT '开关(Switch-默认开):0=关,1=开',
    `describe` text COLLATE utf8mb4_unicode_ci COMMENT '描述(Textarea)',
    `array` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT '[{"key":"1","value":"1"}]' COMMENT '配置(Array):key=名称,value=值',
    `refreshtime` int(10) NOT NULL COMMENT '刷新时间(Datetime)',
    `activitytime` datetime DEFAULT NULL COMMENT '活动时间(Datetime)',
    `startdate` date DEFAULT NULL COMMENT '开始日期(Date)',
    `year` year(4) DEFAULT NULL COMMENT '年份(Year)',
    `time` time DEFAULT NULL COMMENT '时间(Time)',
    `timestamp` timestamp NULL DEFAULT NULL COMMENT '时间戳(Timestamp)',
    `genderdata` enum('male','female') COLLATE utf8mb4_unicode_ci DEFAULT 'female' COMMENT '性别(Radio-默认女):male=男,female=女',
    `name_select` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '名称列表(Select):0=名称0,1=名称1,2=名称2',
    `name_selects` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '0,1' COMMENT '名称列表(Selects):0=名称0,1=名称1,2=名称2',
    `content` text COLLATE utf8mb4_unicode_ci COMMENT '内容(Editor)',
    `city` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT '' COMMENT '省市(City)',
    ...
    `files` varchar(1500) COLLATE utf8mb4_unicode_ci DEFAULT '' COMMENT '附件组',
    `weigh` int(10) DEFAULT '0' COMMENT '权重',
    `icon` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT 'ICON选择器',
    `createtime` int(10) DEFAULT NULL COMMENT '创建时间',
    `updatetime` int(10) DEFAULT NULL COMMENT '更新时间',
    `status` enum('normal','hidden') COLLATE utf8mb4_unicode_ci DEFAULT 'normal' COMMENT '状态:normal=正常,hidden=隐藏',
